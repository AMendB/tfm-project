...
def render_2D(img_orig, img_scale, objects, is_tracking_on):
    ...
    for obj in objects.object_list:
        if render_object(obj, is_tracking_on):
            ...
            cv2.line(img_orig, (int(top_left_corner[0]), int(top_left_corner[1])),
                     (int(top_right_corner[0]), int(top_right_corner[1])), base_color, line_thickness)
            cv2.line(img_orig, (int(bottom_left_corner[0]), int(bottom_left_corner[1])),
                     (int(bottom_right_corner[0]), int(bottom_right_corner[1])), base_color, line_thickness)
            # Creation of 2 vertical lines
            cv2.line(img_orig, (int(bottom_left_corner[0]), int(bottom_left_corner[1])),
                     (int(top_left_corner[0]), int(top_left_corner[1])), base_color, line_thickness)
            cv2.line(img_orig, (int(bottom_right_corner[0]), int(bottom_right_corner[1])),
                     (int(top_right_corner[0]), int(top_right_corner[1])), base_color, line_thickness)

            # Display Object label as text
            position_image = get_image_position(obj.bounding_box_2d, img_scale)
            text_position = (int(position_image[0] - 60), int(position_image[1] - 42))
            font=cv2.FONT_HERSHEY_COMPLEX_SMALL
            font_scale=0.8
            font_thickness=3
            text_color = (0, 0, 0, 0)            
            cv2.putText(img_orig, text, text_position, font, font_scale, text_color, font_thickness)
            font_thickness=1
            ...
            if np.isfinite(obj.position[2]):
                ...
                text_position = (int(position_image[0] - 60), int(position_image[1] - 25))
                font_thickness=3
                text_color = (0, 0, 0, 0)   
                cv2.putText(img_orig, text, text_position, font, font_scale, text_color, font_thickness)
                font_thickness=1
    ...
    cv2.addWeighted(img_orig, 0.7, overlay, 0.3, 0.0, img_orig)
