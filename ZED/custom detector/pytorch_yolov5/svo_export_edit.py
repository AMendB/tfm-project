...
def main():
    ...
    if not sys.argv or len(sys.argv) != 4:
        ...
        sys.stdout.write("                   5=Export LEFT only AVI.\n")
        ...
        sys.stdout.write("  (SEQUENCE LEFT+DEPTH_16Bit):  ZED_SVO_Export \"path/to/file.svo\" \"path/to/output/folder\""
                         " 4\n")
        sys.stdout.write("  (AVI LEFT_ONLY):  ZED_SVO_Export \"path/to/file.svo\" \"path/to/output/file.avi\" 5\n")
        exit()
...
#                                       cv2.VideoWriter_fourcc('M', '4', 'S', '2'),
    ...
    while True:
        if zed.grab(rt_param) == sl.ERROR_CODE.SUCCESS:
            ...
            elif app_type == AppType.LEFT_ONLY:
                pass
                
            if output_as_video:
                if app_type == AppType.LEFT_ONLY:
                    # Copy the left image to SBS image
